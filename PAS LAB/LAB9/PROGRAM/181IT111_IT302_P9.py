...
class solution(object):
	def call(self,Y,X,Z):
		...
		file.write("\n")
		print("P("+str(Z)+") = "+"{:.17f}".format(float(ans)))
		file.write("P("+str(Z)+") = "+"{:.17f}".format(float(ans)))
		file.write("\n")
		print("----------------------------------------------------------------------------------------------------------------------------------------------- \n")
		file.write("-----------------------------------------------------------------------------------------------------------------------------------")
		file.write("\n")
		...
		print("Continuity Correction Factor = 0.5 \n")
		file.write("Continuity Correction Factor = 0.5 \n \n")
		b1=a1-u
		b1=b1/s
		b1=round(b1, 2)
		b2=a2-u
		...
		print("P("+str(b1)+"  <= x <=  "+str(b2)+") = "+str(q2)+" - "+str(q1)+" = "+"{:.17f}".format(float(ans2)))
		file.write("P("+str(b1)+"  <= x <=  "+str(b2)+") = "+str(q2)+" - "+str(q1)+" = "+"{:.17f}".format(float(ans2))+"\n")
		print("P("+str(Z)+") = "+"{:.17f}".format(float(ans2))+"\n")
		file.write("P("+str(Z)+") = "+"{:.17f}".format(float(ans2)))
		file.write("\n")
		error=ans-ans2
		print("Error = "+"{:.17f}".format(float(ans))+" - "+"{:.17f}".format(float(ans2))+" = "+"{:.17f}".format(float(error))+"\n \n")
		file.write("Error = "+"{:.17f}".format(float(ans))+" - "+"{:.17f}".format(float(ans2))+" = "+"{:.17f}".format(float(error))+"\n \n")
		print("-----------------------------------------------------------------------------------------------------------------------------------------------\n")
		file.write("----------------------------------------------------------------------------------------------------------------------------------- \n")
		print("The error due to using the normal approximation is "+"{:.17f}".format(float(error)))
		file.write("The error due to using the normal approximation is "+"{:.17f}".format(float(error)))
	...
	if g[0][1].isdigit() and g[0][2].isdigit():
		...
		if X<0 or Y<0 or Z<0:
			...
			if Y*y>5:
				...
				file = open("output.txt","w")
				file.write("n*p < 5 so we cant consider Normal Approximation of The Binomial Distribution ")
				print("n*p < 5 so we cant consider Normal Approximation of The Binomial Distribution ")
