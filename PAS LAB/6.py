class solution(object):
	def call(self,n,oranges,apples,banana):
		...
		w=[]
		for x in range(apples+1):
			for y in range(oranges+1):
				if x+y<=n:
					w.append([x,y])
		...
		file.write("---------------------------------------------------------------------------------------------\n")
		print(" probability function f(x, y) = ("+str(apples)+'C'+"x"+"*"+str(banana)+'C'+"y"+"*"+str(oranges)+'C'+'('+str(n)+'-x-y)) /'+str(total)+"C"+str(n)+"\n \n" )
		file.write(" probability function f(x, y) = ("+str(apples)+'C'+"x"+"*"+str(banana)+'C'+"y"+"*"+str(oranges)+'C'+'('+str(n)+'-x-y)) /'+str(total)+"C"+str(n)+"\n \n" )
		print(" probability function f(x, y) = ("+str(apples)+'C'+"x"+"*"+str(banana)+'C'+"y"+"*"+str(oranges)+'C'+'('+str(n)+'-x-y)) /'+str(total)+"C"+str(n)+"\n \n" )
		...
		print(w)
		for x in w:
			e=self.c(apples,x[0])*self.c(banana,x[1])*self.c(oranges,n-(x[1]+x[0]))
			h=e/(1.0*M)
			c=n-(x[1]+x[0])
			file.write("Number  of ways to  select  "+str(x[0])+" apples fruits, "+str(x[1])+" banana fruits and "+str(c)+" oranges fruits from  "+str(total)+" fruits is "+str(apples)+'C'+str(x[0])+"*"+str(banana)+'C'+str(x[1])+"*"+str(oranges)+'C'+str(c)+" = "+str(e)+"\n")
			file.write("Probability of  selecting "+str(x[0])+" apples fruits, "+str(x[1])+" banana fruits and "+str(c)+" oranges fruits from  "+str(total)+" fruits is "+str(e)+"/"+str(M)+" = "+str(h)+"\n \n")
			d[x[0]][x[1]]=h
